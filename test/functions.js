...
function getAuctionContractDeployed() {
  var addresses = [];
  ...
  if (factoryContractAddress != null && factoryContractAbi != null) {
    ...
    var auctionDeployedEvents = contract.AuctionDeployed({}, { fromBlock: factoryFromBlock, toBlock: latestBlock });
    i = 0;
    auctionDeployedEvents.watch(function (error, result) {
      console.log("RESULT: get AuctionDeployed " + i++ + " #" + result.blockNumber + " " + JSON.stringify(result.args));
      addresses.push(result.args.auction);
    });
    auctionDeployedEvents.stopWatching();
  }
  return addresses;
}
...
function printFactoryContractDetails() {
  ...
  if (factoryContractAddress != null && factoryContractAbi != null) {
    ...
    i = 0;
    auctionDeployedEvents.watch(function (error, result) {
      console.log("RESULT: AuctionDeployed " + i++ + " #" + result.blockNumber + " " + JSON.stringify(result.args));
    });
    auctionDeployedEvents.stopWatching();
...
// Auction Contract
//-----------------------------------------------------------------------------
var auctionContractAddress = null;
var auctionContractAbi = null;

function addAuctionContractAddressAndAbi(address, abi) {
  auctionContractAddress = address;
  auctionContractAbi = abi;
}

var auctionFromBlock = 0;

function printAuctionContractDetails() {
  console.log("RESULT: auctionContractAddress=" + auctionContractAddress);
  if (auctionContractAddress != null && auctionContractAbi != null) {
    ...
    var ownershipTransferredEvents = contract.OwnershipTransferred({}, { fromBlock: auctionFromBlock, toBlock: latestBlock });
    i = 0;
    ownershipTransferredEvents.watch(function (error, result) {
      console.log("RESULT: OwnershipTransferred " + i++ + " #" + result.blockNumber + " " + JSON.stringify(result.args));
    });
    ...
    auctionFromBlock = latestBlock + 1;
  }
}
